...
class Flux2Transformer2DModel(
    ...
    def __init__(
        ...
        if video_editing:
            gn_groups = _get_gn_groups(in_channels)
            proj_dim = patch_size * patch_size * self.out_channels
            proj_gn_groups = _get_gn_groups(proj_dim)
            proj_gn_groups_2 = _get_gn_groups(proj_dim * 2)
            self.hidden_state_down = nn.Sequential(
                nn.Conv3d(in_channels, in_channels * 2, kernel_size=3, stride=2, padding=1, bias=False),
                nn.GroupNorm(gn_groups, in_channels * 2, eps=1e-6),
                nn.SiLU(),
                nn.Conv3d(in_channels * 2, in_channels, kernel_size=3, stride=2, padding=1, bias=False),
                nn.GroupNorm(gn_groups, in_channels, eps=1e-6),
            )
            self.hidden_state_up = nn.Sequential(
                nn.ConvTranspose3d(proj_dim, proj_dim * 2, kernel_size=4, stride=2, padding=1, bias=False),
                nn.GroupNorm(proj_gn_groups_2, proj_dim * 2, eps=1e-6),
                nn.SiLU(),
                nn.ConvTranspose3d(proj_dim * 2, proj_dim, kernel_size=4, stride=2, padding=1, bias=False),
                nn.GroupNorm(proj_gn_groups, proj_dim, eps=1e-6),
            )
    ...
    def forward(
        ...
        use_3d_compression = hidden_states.ndim == 5
        if use_3d_compression and not self.video_editing:
            raise ValueError("Received 5D hidden_states but video_editing is disabled.")
        if use_3d_compression:
            bsz, channels, t, h, w = hidden_states.shape
            t_pad = (-t) % 4
            h_pad = (-h) % 4
            w_pad = (-w) % 4
            if t_pad or h_pad or w_pad:
                hidden_states = F.pad(hidden_states, (0, w_pad, 0, h_pad, 0, t_pad))
            if not hasattr(self, "hidden_state_down"):
                raise ValueError("3D compression requested but video_editing layers are not initialized.")
            hidden_states = self.hidden_state_down(hidden_states)
            t4, h4, w4 = hidden_states.shape[2], hidden_states.shape[3], hidden_states.shape[4]
            hidden_states = hidden_states.permute(0, 2, 3, 4, 1).reshape(bsz, t4 * h4 * w4, channels)
        elif hidden_states.ndim != 3:
            raise ValueError(f"Expected hidden_states to be 3D or 5D, got {hidden_states.ndim}D.")
        ...
        if use_3d_compression:
            expected_full = t * h * w
            expected_down = t4 * h4 * w4
            if img_ids.shape[0] == expected_full:
                img_ids = img_ids.reshape(t, h, w, -1)
                if t_pad:
                    pad_t = img_ids[-1:, :, :, :].expand(t_pad, h, w, img_ids.shape[-1])
                    img_ids = torch.cat([img_ids, pad_t], dim=0)
                if h_pad:
                    pad_h = img_ids[:, -1:, :, :].expand(img_ids.shape[0], h_pad, w, img_ids.shape[-1])
                    img_ids = torch.cat([img_ids, pad_h], dim=1)
                if w_pad:
                    pad_w = img_ids[:, :, -1:, :].expand(img_ids.shape[0], img_ids.shape[1], w_pad, img_ids.shape[-1])
                    img_ids = torch.cat([img_ids, pad_w], dim=2)
                img_ids = img_ids[::4, ::4, ::4].reshape(expected_down, -1)
            elif img_ids.shape[0] != expected_down:
                raise ValueError(
                    f"Expected img_ids length to be {expected_full} or {expected_down} for 3D compression, "
                    f"got {img_ids.shape[0]}."
                )
        ...
        if use_3d_compression:
            ...
            output = output.reshape(bsz, t4, h4, w4, out_channels).permute(0, 4, 1, 2, 3)[:, :, t4//2:, :, :]
            output = self.hidden_state_up(output)
            # print(output.shape, t, h, w)
            if t_pad or h_pad or w_pad:
                output = output[:, :, :t//2 , :h, :w]
